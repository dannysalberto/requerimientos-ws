...
public interface ObrasRepository extends JpaRepository<Obra, Integer>{
	...
	@Query("SELECT SUM(m.valortotalactividadaiu) FROM ActividadobraWS m where m.obra.intcodigoobra = ?1")
	double totalActividades(Integer idObra);
	
	@Query("SELECT SUM(m.floatcantplanifao*m.valorunitario) FROM ActividadobraWS m where m.obra.intcodigoobra = ?1")
